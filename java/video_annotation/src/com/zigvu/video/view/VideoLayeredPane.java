...
public class VideoLayeredPane extends JLayeredPane {
	...
	private JLabel capturedFrameLabel;
	...
	public void setCapturedFrame(ImageIcon img){
		capturedFrameLabel = new JLabel(img);
		capturedFrameLabel.setBounds(0, 0, paneWidth, paneHeight);
		this.add(capturedFrameLabel, new Integer(1));
		this.validate();
		this.repaint();
	}
	
	public void removeCapturedFrame(){
		try {
			this.remove(capturedFrameLabel);
		} catch (Exception e){
			// do nothing for now
		}
		this.validate();
	...
	protected void addPointToNewPoly(int x, int y){
		...
		if (creatingNewPolyToAdd){
			...
			newPolyToAdd = new ResizablePolygon(paneWidth, paneHeight, curLogoColor, curLogoIndex, curLogoName);
